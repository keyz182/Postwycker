...
echo $PSQLCONTAINER
#COLLECTOR_CONTAINER=$(
docker run --rm -t -i --name $INSTANCE_NAME-collector --link $INSTANCE_NAME-postgres:postgis21 \
    ...
    -e TWITTER_OAUTH_TOKEN_SECRET="$TWITTER_OAUTH_TOKEN_SECRET" postwycker /bin/bash
